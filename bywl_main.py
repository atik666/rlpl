...
class BYWL(nn.Module):
    def __init__(
        ...
        # number of augmentation to be used. Default = 1.
        self.n_aug = n_aug

        # select the methods to execute
        self.method = method

        # Weight parameter of the auxiliary loss 
    ...
    def forward(
        ...
        for i in range(self.n_aug):
            ...
            img_one.append(image_one)
            img_two.append(image_two)

        image_one = torch.cat(img_one, dim=0)
        image_two = torch.cat(img_two, dim=0)
